...
class Config:
    ...
    # Condescending behavior detection
    condescension_enabled: bool = True  # Detect mansplaining/condescending patterns
    condescension_mute_threshold: int = 2  # Mute after N condescending messages
    condescension_counts_as_infraction: bool = True  # Count toward block threshold
...
class InfractionTracker:
    ...
    def add_infraction(self, user_acct: str, classification: str, reason: str, is_condescending: bool = False):
        ...
        if is_condescending:
            self.infractions[user_acct]['condescending_count'] += 1
    ...
    def get_condescending_count(self, user_acct: str) -> int:
        return self.infractions.get(user_acct, {}).get('condescending_count', 0)
...
ALSO evaluate for 'CONDESCENDING' behavior patterns:
- Unsolicited over-explanation of basic concepts
- Assuming ignorance without evidence ("Actually, let me explain...")
- Dismissive of expertise ("You clearly don't understand...")
- Patronizing tone ("If you knew anything about X...")
- "Well actually" style corrections on minor points
...
- "is_condescending": true/false - whether the message exhibits condescending patterns
- "condescension_indicators": List of specific phrases/patterns detected (empty array if none)
- "reason": A brief explanation (max 100 chars).
- "confidence": A number from 0.0 to 1.0 indicating your confidence.

Example:
{
  "classification": "MILDLY_NEGATIVE",
  "is_condescending": true,
  "condescension_indicators": ["unsolicited explanation", "assumes ignorance", "dismissive tone"],
  "reason": "Patronizing explanation of basic concept",
  "confidence": 0.85
}
"""
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                required_keys = {'classification', 'reason', 'confidence', 'is_condescending', 'condescension_indicators'}
                ...
                if ai_decision['is_condescending']:
                    self.logger.warning(f"🎓 CONDESCENDING behavior detected: {ai_decision['condescension_indicators']}")
...
class ActionHandler:
    ...
    def take_action(self, mastodon: Mastodon, author: Dict, decision: Dict, status_id: str = None):
        ...
        is_condescending = decision.get('is_condescending', False)
        condescension_indicators = decision.get('condescension_indicators', [])
        
        # Skip if neutral or low confidence
        if classification == 'NEUTRAL' and not is_condescending:
        ...
        if classification in ['MILDLY_NEGATIVE', 'SEVERELY_NEGATIVE'] or is_condescending:
            self.tracker.add_infraction(author_acct, classification, decision['reason'], is_condescending)
        
        infraction_count = self.tracker.get_count(author_acct)
        condescending_count = self.tracker.get_condescending_count(author_acct)
        ...
        if self.config.send_warnings and status_id:
            if not (is_new and self.config.new_account_no_warning):
                self._send_warning(mastodon, author_acct, status_id, infraction_count, classification, 
                                  is_condescending, condescension_indicators)
        
        # Determine action based on severity, history, and condescension
        action = self._determine_action(classification, infraction_count, condescending_count)
        
        if action == 'none':
            if is_condescending:
                self.logger.info(f"Tracking condescending behavior #{condescending_count} for @{author_acct}")
            else:
    ...
    def _determine_action(self, classification: str, infraction_count: int, condescending_count: int = 0) -> str:
        """Determine what action to take based on classification and history"""
        
        # Check condescension threshold if enabled
        if self.config.condescension_enabled and condescending_count >= self.config.condescension_mute_threshold:
            return 'mute'
